...
class NodeHttpServer {
  constructor(config) {
    ...
    if (config.http.webroot) {
      if(config.http.dir){
        app.use(config.http.dir, Express.static(config.http.webroot));
      }else{        
        app.use(Express.static(config.http.webroot));
      }
    }    
    //YS edit
    app.get(config.http.dir + "/getReal", async (req, res) => {
      try {
        const query = req.query;
        const t = await axios.get("https://www.jsene.com/Farglory/Construction/lonzhong94/index.html/getReal",{
          params:{
            stid: query.stid,
            list: query.list
          }
